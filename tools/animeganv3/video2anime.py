...
def parse_args():
    ...
    parser.add_argument('-d','--device', type=str, default='gpu', choices=["cpu","gpu","trt","dml"] ,help='running device')
...
class Cartoonizer():
    def __init__(self, arg):
        self.args = arg
        if self.args.device == "dml":
            self.sess_land = ort.InferenceSession(self.args.model_path, providers = ['DmlExecutionProvider','CPUExecutionProvider'])
        elif ort.get_device() == 'GPU' and self.args.device=="gpu" :
